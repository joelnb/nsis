...
my $User_Map_File = "/home/groups/n/ns/nsis/bin/user.map";
...
$header_line = 
sprintf ("<li><p>%4u-%02u-%02u${wday} %02u:%02u  %s",
		 $year+1900, $mon+1, $mday, $hour, $min, $author);
  }
  ...
  else  # do wrapping, either FSF-style or regular
  {
	...
	$files_last_line_len += 1;  # for $After_Header
      ...
      $msg = &wrap_log_entry
	  ($msg, "\t", 110 - $files_last_line_len, 110);
      $body = $files . $After_Header . $msg;
    }
    else  # not FSF-style
    {
...
$beauty .= "<a href=\"".$baselink.$$qunkref{'filename'}."\">";
}
}
else {
$beauty .= "<a href=\"".$baselink.$$qunkref{'filename'}."\">";
...
$beauty = "</p><p class=\"clitem\">$beauty:";
